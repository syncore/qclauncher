...
// added by syncore: 07/21/17
func initStyledMainWindow(mw *MainWindow, style uint32) error {
	return InitWindow(
		mw,
		nil,
...
// NewStyledMainWindow added by syncore: 07/21/17
func NewStyledMainWindow(style uint32) (*MainWindow, error) {
	if style == 0 {
	...
	if err := initStyledMainWindow(mw, style); err != nil {
		return nil, err
	}
...
// NewMainWindow added by syncore: 07/21/17
func NewMainWindow() (*MainWindow, error) {
	mw := new(MainWindow)

	if err := initStyledMainWindow(mw, win.WS_OVERLAPPEDWINDOW); err != nil {
		return nil, err
	}
